...
class FVFU:
    ...
    def _init_op_handlers(self):
        ...
        self.op_handlers = {
            ...
            "INTtoFP": self._handle_int_to_fp,
            "FPtoINT": self._handle_fp_to_int,
        }

    def calculate(
        self,
        opcode: FVFUOpType,
        a: Union[NDArray[np.float32], NDArray[np.int32], int],
        b: Optional[Union[NDArray[np.float32], NDArray[np.int32], int]] = None,
        ...
        - Conversion ops:
          * INTtoFP: accepts INT32 input and returns FP32 output.
          * FPtoINT: accepts FP32 input and returns INT32 output.
        """
        ...
        if opcode in ["mul", "div"]:
            ...
            operation_type = "act"
        elif opcode in ["INTtoFP", "FPtoINT"]:
    ...
    def _handle_add(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        return a + b

    def _handle_sub(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        return a - b

    def _handle_mul(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        return a * b

    def _handle_div(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        if np.any(b == 0):
            raise ZeroDivisionError("Division by zero")
        return a / b

    def _handle_min(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        return np.minimum(a, b)

    def _handle_max(self, a: NDArray[np.float32], b: NDArray[np.float32]) -> NDArray[np.float32]:
        return np.maximum(a, b)

    def _handle_sig(self, a: NDArray[np.float32], _) -> NDArray[np.float32]:
        # Calculate sigmoid
        return 1 / (1 + np.exp(-a))

    def _handle_tanh(self, a: NDArray[np.float32], _) -> NDArray[np.float32]:
        # Calculate tanh
        return np.tanh(a)

    def _handle_relu(self, a: NDArray[np.float32], _) -> NDArray[np.float32]:
        return np.maximum(0, a)

    def _handle_int_to_fp(self, a: NDArray[np.int32], _) -> NDArray[np.float32]:
        return a.astype(np.float32)

    def _handle_fp_to_int(self, a: NDArray[np.float32], _) -> NDArray[np.int32]:
